...
Int_t plots_merge_hist_files_Aperture(){
  const Int_t n = 8;
  TString fileNarr[n];
  ...
  //
  Double_t constApperture = 5.45*1.0e-9*4*4*TMath::Pi()*TMath::Pi()*6371.0*6371.0;
  //
  gr_eff_vs_e->SetPoint(pointID,10,1.0e-9*constApperture);
  //gr_eff_vs_e->SetPointError(pointID,0.0,1.0e-9/10.0);
  gr_eff_vs_e->SetPointError(pointID,0.0,0.0);
  //
  pointID = gr_eff_vs_e->GetN();
  gr_eff_vs_e->SetPoint(pointID,35,h1[1]->GetBinContent(8)*constApperture);
  //gr_eff_vs_e->SetPointError(pointID,0.0,h1[1]->GetBinContent(8)/3.0);
  gr_eff_vs_e->SetPointError(pointID,0.0,0.0);
  //
  pointID = gr_eff_vs_e->GetN();
  gr_eff_vs_e->SetPoint(pointID,70,h1[2]->GetBinContent(8)*constApperture);
  //gr_eff_vs_e->SetPointError(pointID,0.0,h1[2]->GetBinContent(8)/3.0);
  gr_eff_vs_e->SetPointError(pointID,0.0,0.0);
  //
  pointID = gr_eff_vs_e->GetN();
  gr_eff_vs_e->SetPoint(pointID,100,h1[3]->GetBinContent(8)*constApperture);
  //gr_eff_vs_e->SetPointError(pointID,0.0,h1[3]->GetBinContent(8)/3.0);
  gr_eff_vs_e->SetPointError(pointID,0.0,0.0);
  //
  pointID = gr_eff_vs_e->GetN();
  gr_eff_vs_e->SetPoint(pointID,350,h1[4]->GetBinContent(8)*constApperture);
  //gr_eff_vs_e->SetPointError(pointID,0.0,h1[4]->GetBinContent(8)/100.0);
  gr_eff_vs_e->SetPointError(pointID,0.0,0.0);
  //
  pointID = gr_eff_vs_e->GetN();
  gr_eff_vs_e->SetPoint(pointID,700,h1[5]->GetBinContent(8)*constApperture);
  //gr_eff_vs_e->SetPointError(pointID,0.0,h1[5]->GetBinContent(8)/100.0);
  gr_eff_vs_e->SetPointError(pointID,0.0,0.0);
  //
  pointID = gr_eff_vs_e->GetN();
  gr_eff_vs_e->SetPoint(pointID,1000,h1[6]->GetBinContent(8)*constApperture);
  //gr_eff_vs_e->SetPointError(pointID,0.0,h1[6]->GetBinContent(8)/100.0);
  gr_eff_vs_e->SetPointError(pointID,0.0,0.0);
  //
  pointID = gr_eff_vs_e->GetN();
  gr_eff_vs_e->SetPoint(pointID,3000,h1[7]->GetBinContent(8)*constApperture);
  ...
  TGraph *gr_App_poemma_vs_e = new TGraph();
  gr_App_poemma_vs_e->SetName("gr_App_poemma_vs_e");
  gr_App_poemma_vs_e->SetTitle("gr_App_poemma_vs_e");
  //gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,15.89849108367627)/1.0e+15,  0.35171885621935933*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());
  gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,15.994513031550069)/1.0e+15, 0.8033250400381446*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());
  gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,16.255144032921812)/1.0e+15, 1.7671874094791553*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());
  gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,17.00960219478738)/1.0e+15,  8.083672227438296*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());
  gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,17.75034293552812)/1.0e+15,  25.403368279664964*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());
  gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,18.799725651577504)/1.0e+15, 86.05628792983362*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());
  gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,19.272976680384087)/1.0e+15, 148.31861185760408*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());
  gr_App_poemma_vs_e->SetPoint(gr_App_poemma_vs_e->GetN(), TMath::Power(10,19.835390946502056)/1.0e+15, 246.20924014946254*30.0/180.0*TMath::Pi()/2.0/TMath::Pi());

  gr_App_poemma_vs_e->SetMarkerStyle(20);
  gr_App_poemma_vs_e->SetMarkerColor(kRed);
  gr_App_poemma_vs_e->SetLineColor(kRed);
  gr_App_poemma_vs_e->SetLineWidth(3.0);
  
  TMultiGraph *mg03 = new TMultiGraph();
  mg03->Add(gr_eff_vs_e);
  mg03->Add(gr_App_poemma_vs_e);
  mg03->GetXaxis()->SetRangeUser(0.01,70000);
  mg03->SetMinimum(1.0e-9);
  mg03->Draw("APL");
  mg03->GetXaxis()->SetRangeUser(0.01,70000);
  mg03->GetXaxis()->SetTitle("Energy, PeV");
  mg03->GetYaxis()->SetTitle("Aperture, km*km*sr");

  TLegend *leg000 = new TLegend(0.6,0.3,0.9,0.6,"","brNDC");
  leg000->AddEntry(gr_eff_vs_e, "Terzina", "pl");
  leg000->AddEntry(gr_App_poemma_vs_e, "POEMMA at 30 deg FoV", "pl");
  leg000->Draw();
  
  //////////////////////////////////////////////////////

  TGraph *gr_poemma_vs_terzina = new TGraph();
  gr_poemma_vs_terzina->SetName("gr_poemma_vs_terzina");
  gr_poemma_vs_terzina->SetTitle("");
  //
  Double_t eee;
  eee = 100;
  gr_poemma_vs_terzina->SetPoint( gr_poemma_vs_terzina->GetN(),eee,gr_eff_vs_e->Eval(eee)/gr_App_poemma_vs_e->Eval(eee));
  eee = 300;
  gr_poemma_vs_terzina->SetPoint( gr_poemma_vs_terzina->GetN(),eee,gr_eff_vs_e->Eval(eee)/gr_App_poemma_vs_e->Eval(eee));
  eee = 700;
  gr_poemma_vs_terzina->SetPoint( gr_poemma_vs_terzina->GetN(),eee,gr_eff_vs_e->Eval(eee)/gr_App_poemma_vs_e->Eval(eee));
  ...
  gr_poemma_vs_terzina->SetMarkerStyle(20);
  gr_poemma_vs_terzina->SetMarkerColor(kBlack);
  gr_poemma_vs_terzina->SetLineColor(kBlack);
  gr_poemma_vs_terzina->SetLineWidth(3.0);

  TCanvas *c002 = new TCanvas("c002","c002",10,10,1200,800);
  gStyle->SetPalette(1);
  gStyle->SetFrameBorderMode(0);
  gROOT->ForceStyle();
  gStyle->SetOptStat(kFALSE);
  //
  c002->SetRightMargin(0.01);
  c002->SetLeftMargin(0.08);
  c002->SetTopMargin(0.01);
  c002->SetBottomMargin(0.08);

  gPad->SetGridx();
  gPad->SetGridy();

  gr_poemma_vs_terzina->SetMinimum(0.0);
  gr_poemma_vs_terzina->SetMaximum(1.0);
  
  gr_poemma_vs_terzina->Draw("APL");

  gr_poemma_vs_terzina->GetXaxis()->SetTitle("Energy, PeV");
  gr_poemma_vs_terzina->GetYaxis()->SetTitle("Aperture ratio - Terzina/POEMMA");
